         ...
         <div class="row" style="height:640px;">
            @foreach($fallWordPosts as $fallWordPost)
                <div class="" style=" margin-right: 15px;">
                    <table class="">
                        <tr>
                            <p class="text-nowrap" style="line-height: 1.0;  font-weight:1000; padding-top: 10px; padding-left: 10px; padding-right: 30px;">{{$fallWordPost->name}}</p>
                        </tr>
                        <tr>
                            @isset ($fallWordPost->photo)
                            <td style="padding-top: 20px;">
                                <a href="{{ Storage::disk('s3')->url("{$fallWordPost->photo}") }}" data-lightbox="group">
                                    <img src="{{ Storage::disk('s3')->url("{$fallWordPost->photo}") }}" class="" width="193" height="130"/>
                                </a>
                            </td>
                            @endisset
                            
                            <td class="" style="height: 450px; line-height: 1.0; font-weight:800; padding: 20px 10px 10px 10px;">{{$fallWordPost->explain}}</td>
                        </tr>
                        <tr>
            ...
            @endforeach
            <div class="row" style="height:640px;">         
                @foreach($fallWordPost->posts as $post )
                <div class="" style="margin-right: 15px;">
